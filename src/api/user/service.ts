...
export const getUserProfileFromOpenSea = async (id: string): Promise<OpenSeaAccount> => {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (axiosError.response) {
          ...
          const errorData = axiosError.response.data;
           if ((status === 400 || status === 404) && errorData?.errors?.some(
                 (err: string) =>
                   err.includes("not found") || err.includes("Address or username") || err.includes("Account not found")
             )) {
          ...
          console.error('OpenSea API Error: No response received.', axiosError.request);
